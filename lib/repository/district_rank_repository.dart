...
class DistrictRankRepository {
  ...
  Future<DistrictRankModel> fetchModel(int team, int year) async {
    ...
    try {
      await _fetchDistrictKey(team).then(
        (districtList) async {
          ...
          DistrictList districtForThisYear = districtList.firstWhere(
            (element) => year == element.year,
            orElse: () {
              int closestYear =
                  _closestYear(year, districtList.map((e) => e.year));
              throw FetchException(
  ...
  Future<Team> _fetchTeamAbout(int team) async {
    final response = await tbaApi.getTeamApi().getTeam(teamKey: 'frc$team');
    return response.data!;
  }

  Future<List<Award>> _fetchTeamAwards(
    int team,
    int year,
  ) async {
  ...
  Future<List<Media>> _fetchTeamAvatar(
    int team,
    int year,
  ) async {
    final response = await tbaApi
  ...
  int _closestYear(
    ...
    for (int checkYear in yearsRanked) {
      if ((checkYear - currentYear).abs() < smallestDistYear.dist) {
        smallestDistYear = (
          dist: (checkYear - currentYear).abs(),
          year: checkYear,
        );
      }
    }
